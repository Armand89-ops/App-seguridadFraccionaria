...
const Chats = ({ navigation, route }) => {
  ...
  // Cargar chats donde participa el residente
  useEffect(() => {
    if (!idResidente) return;
    fetch(`http://192.168.0.103:3000/verChatsResidente/${idResidente}`)
      .then(res => res.json())
  ...
  if (chatSeleccionado) {
    ...
    return (
      <SafeAreaView style={styles.container}>
        <MenuResidente  navigation={navigation} idResidente={idResidente} titulo="Chat" />
        <View style={styles.content}>
          <Text style={styles.screenTitle}>
            {chatSeleccionado.tipo === 'general'
              ...
              : `Chat Privado (${obtenerNombresUsuarios(chatSeleccionado.usuarios.filter(u => u !== idResidente))})`}
          </Text>
          <FlatList
            data={mensajes}
            ...
            renderItem={({ item }) => (
              <View style={[
                styles.mensaje,
                item.idUsuario === idResidente ? styles.mensajePropio : styles.mensajeOtro
              ]}>
  ...
  return (
    <SafeAreaView style={styles.container}>
      <MenuResidente navigation={navigation} idResidente={idResidente} titulo="Chats" />
      <View style={styles.content}>
        <FlatList
          data={chats}
          keyExtractor={item => item._id}
            ...
            >
              <Text style={styles.chatTitle}>
                {item.tipo === 'general'
                  ...
                  : `Chat Privado (${obtenerNombresUsuarios(item.usuarios.filter(u => u !== idResidente))})`}
              </Text>
            </TouchableOpacity>
          )}
        />
